...
require_once __DIR__ . '/../libs/correo_cita.php'; // def: enviarCorreoCita($correo,$nombre,$detalle,$creadoPor,$tele=null)
...
try {
    ...
    // -------- Entrada
    $cedula            = trim((string)($body['cedula_paciente'] ?? ''));
    ...
    $origen_id         = (int)($body['origen_id'] ?? 3); // 3 = Web (ajústalo si quieres)
    ...
    if ($paciente_id <= 0) {
        ...
        $pac = $citaModel->buscarPacientePorCedula($cedula);
        if (!$pac) { http_response_code(404); echo json_encode(['estado'=>'error','mensaje'=>'Paciente no encontrado']); exit; }
        ...
        if (!$pac) { http_response_code(404); echo json_encode(['estado'=>'error','mensaje'=>'Paciente no válido']); exit; }
    }
    ...
    if (!$med) { http_response_code(404); echo json_encode(['estado'=>'error','mensaje'=>'Médico no válido']); exit; }
    ...
    foreach ($catReqs as $c) {
        $q = $pdo->prepare($c['sql']); $q->execute([':id'=>$c['id']]);
        if (!$q->fetchColumn()) { http_response_code(400); echo json_encode(['estado'=>'error','mensaje'=>$c['msg']]); exit; }
    }
    ...
    if (!$turnos) { http_response_code(409); echo json_encode(['estado'=>'error','mensaje'=>'El médico no atiende el día seleccionado']); exit; }
    ...
    if (!$enTurno) { http_response_code(409); echo json_encode(['estado'=>'error','mensaje'=>'La hora no está dentro del turno del médico']); exit; }

    $ocupadas = $citaModel->obtenerHorasOcupadas($medico_id, $fecha);
    if (in_array($hhmm, $ocupadas, true)) { http_response_code(409); echo json_encode(['estado'=>'error','mensaje'=>'La hora ya está reservada']); exit; }

    // -------- Crear cita (+ telecita si aplica)
    $pdo->beginTransaction();

    // re-chequeo de carrera
    ...
    if ($check->fetchColumn()) { $pdo->rollBack(); http_response_code(409); echo json_encode(['estado'=>'error','mensaje'=>'La hora fue tomada por otra operación.']); exit; }
    ...
    if (!$ok) { $pdo->rollBack(); http_response_code(500); echo json_encode(['estado'=>'error','mensaje'=>'No se pudo guardar la cita']); exit; }
    ...
    $teleData = null;
    if ($modalidad === 'online') {
        // base del enlace de triaje (tomado del sistema web)
